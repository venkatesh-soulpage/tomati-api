...
const getLocations = async (req, res, next) => {
  try {
    ...
    if (!collaborator && scope !== "ADMIN")
      return res.status(400).json("Invalid account").send();
    ...
    const locations = await models.Location.query()
      ...
      .modify((builder) => {
        ...
        if (collaborator && collaborator.organization) {
          const locations = collaborator.organization.locations.map(
...
const getChildrenLocations = async (req, res, next) => {
  try {
    ...
    const available_locations = await models.Location.query()
      .withGraphFetched(
        `[
                        childrens.[
                            childrens
...
const createLocation = async (req, res, next) => {
  try {
    ...
    const location = await models.Location.query().insert({
      name,
      is_country,
      parent_location,
      passport_available,
...
const deleteLocation = async (req, res, next) => {
  try {
    ...
    const location = await models.Location.query()
      .withGraphFetched(
        `[
          childrens
      ]`
    ...
    let ids = _.map(location.childrens, "id");
    ids.push(parseInt(location_id));

    await models.Location.query()
      .delete()
      .whereIn("parent_location", ids)
      .orWhereIn("id", ids);
    await models.Location.query().deleteById(location_id);

    return res.status(201).json("Location deleted successfully").send();
  } catch (e) {
    console.log(e);
    return res.status(500).json(JSON.stringify(e)).send();
  }
