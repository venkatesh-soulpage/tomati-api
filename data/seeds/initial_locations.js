...
exports.seed = async (knex) => {
  ...
  await knex("locations").insert(countries_data);

  // Add all the states
  const countries = await knex("locations").where({ is_country: true });
  for (const country of countries) {
    const country_states = states_data
      .filter((state) => state.parent_name === country.name)
      .map((state) => {
        return {
          name: state.name,
          is_country: false,
    ...
    await knex("locations").insert(country_states);
  }

  // Add all the cities
  const states = await knex("locations").where({ is_country: false });
  for (const state of states) {
    const state_cities = cities_data
      .filter((city) => city.parent_name === state.name)
      .map((city) => {
        return {
          name: city.name,
          is_country: false,
    ...
    await knex("locations").insert(state_cities);
  }
};
